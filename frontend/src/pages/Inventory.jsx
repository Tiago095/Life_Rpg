...
export default function Inventory() {
  ...
  const handleConsume = async (item) => {
    try {
      ...
      // Actualizar activeConsumables localmente
  ...
  const isItemEquipped = (itemId) =>
    Object.values(equippedSlots).some(item => item?.id === itemId)

  const getEquippedSlot = (itemId) =>
    Object.entries(equippedSlots).find(([_, item]) => item?.id === itemId)?.[0]

  const isConsumableActive = (itemId) =>
    activeConsumables.some(a => a.id === itemId)

  const getActiveCharges = (itemId) =>
    activeConsumables.find(a => a.id === itemId)?.activeCharges ?? 0

  // Quantidade ainda no inventário (para mostrar no botão)
  const getInventoryQty = (itemId) =>
    allItems.find(i => i.id === itemId)?.quantity ?? 0
              ...
              {currentList.map(item => (
                ...
                >
                  ...
                  {item.category === 'consumable' && (
                    <span className={`inv-qty-badge${isConsumableActive(item.id) ? ' inv-qty-badge--active' : ''}`}>
                      {isConsumableActive(item.id)
                        ? `${getActiveCharges(item.id)}⚡ | x${item.quantity}`
                        : `x${item.quantity}`
                      }
                    </span>
                  )}
