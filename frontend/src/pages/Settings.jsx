...
export default function Settings() {
  ...
  const originalAvatar   = sessionStorage.getItem('avatar')   || null 
  const originalLanguage = sessionStorage.getItem('language') || 'English [EN-US]'
  const originalTheme    = sessionStorage.getItem('theme')    || 'Cyberpunk Blue (Default)'
  const originalHighContrast = useRef(sessionStorage.getItem('highContrast') === 'true')
  ...
  const [avatarName, setAvatarName]     = useState(originalAvatar)
  const [avatarSrc, setAvatarSrc]       = useState(originalAvatar ? avatarMap[originalAvatar] : null)
  ...
  const handleHighContrast = (val) => {
    ...
    sessionStorage.setItem('highContrast', val)
    setHighContrast(val)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.user.language) sessionStorage.setItem('language', data.user.language)
        if (data.user.theme)    sessionStorage.setItem('theme',    data.user.theme)
        ...
        updateUser({
          ...
          highContrast: data.user.highContrast,
        })
